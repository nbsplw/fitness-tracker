...
const (
	showTrainingInfoTemplate = "Тип тренировки: %s\nДлительность: %.2f ч.\nДистанция: %.2f км.\nСкорость: %.2f км/ч\nСожгли калорий: %.2f\n"
)
...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Бег":
		dist := distance(action)                                   // вызовите здесь необходимую функцию
		...
		return fmt.Sprintf(showTrainingInfoTemplate, trainingType, duration, dist, speed, calories)
	...
	case trainingType == "Плавание":
		dist := distance(action)                                                   //float64(lengthPool*countPool) / mInKm                              // вызовите здесь необходимую функцию
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	calories := walkingCaloriesWeightMultiplier*weight + (math.Pow(speed, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight
	return calories * duration * minInH
}
